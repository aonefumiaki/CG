...
//  1枚の場のカードに全員が「ハイ/ロー」を賭け → 全員出そろったらめくって一斉判定
...
(function(){
  function init(){
    ...
    var players=[], cur=0, round=1, deck=[], current=null, history=[], phase='bet', over=false;
    ...
    $('hlm-high').addEventListener('click', function(){ bet('high'); });
    $('hlm-low').addEventListener('click', function(){ bet('low'); });
    ...
    function renderBoard(){
      // 手番バー
      var who, meta;
      if(phase==='bet'){ who=players[cur].name+' の予想'; meta=round+' / '+rounds+'周 ・ 賭け '+cur+'/'+players.length; }
      else if(phase==='reveal'){ who='全員そろいました！'; meta=round+' / '+rounds+'周'; }
      else { who='結果'; meta=round+' / '+rounds+'周'; }
      ...
      // ボタン表示の出し分け
      var betPhase=(phase==='bet');
      $('hlm-guess').style.display = betPhase ? 'flex' : 'none';
      $('hlm-high').disabled=!betPhase; $('hlm-low').disabled=!betPhase;
      $('hlm-reveal').classList.toggle('hidden', phase!=='reveal');
      $('hlm-next').classList.toggle('hidden', phase!=='result');
      if(phase==='result') $('hlm-next').textContent = (round>=rounds) ? '結果を見る' : '次のカードへ';
      ...
      $('hlm-hint').textContent =
        phase==='bet' ? players[cur].name+'さん：場のカード（'+C.rankLabel(current.rank)+'）より次が高い(ハイ)か低い(ロー)か予想して、端末を次の人へ。'
        : phase==='reveal' ? '全員の予想がそろいました。「めくる！」を押して一斉に判定します。'
        : '結果が出ました。'+(round>=rounds ? '「結果を見る」で最終順位へ。' : '「次のカードへ」で続けます。');
    }
